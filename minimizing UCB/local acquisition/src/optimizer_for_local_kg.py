...
from src.local_KG import Local_KG, _get_value_function
...
def optimize_acqf_local_kg(
    ...
    **kwargs: Any,
    ...
    if sequential and q > 1:
        ...
        for i in range(q):
            candidate, acq_value = optimize_acqf_local_kg(
    ...
    if batch_initial_conditions is None:
        ...
        #     if isinstance(acq_function, Local_KG)
...
def optimize_acqf_cyclic(
    ...
    candidates, acq_vals = optimize_acqf_local_kg(
        acq_function=acq_function,
        bounds=bounds,
        q=q,
        num_restarts=num_restarts,
    ...
    if q > 1:
        ...
        while not stop:
            for i in range(q):
                ...
                candidate_i, acq_val_i = optimize_acqf_local_kg(
                    acq_function=acq_function,
                    bounds=bounds,
                    q=1,
                    num_restarts=num_restarts,

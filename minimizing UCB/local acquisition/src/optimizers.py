...
class MinimizingUCB(AbstractOptimizer):
    """Optimizer for Bayesian gradient ascent.
    ...
    def __init__(
        ...
        optimize_acqf: Callable[[GradientInformation, torch.Tensor], torch.Tensor],
        optimize_acqf_config: Dict[str, Union[torch.Tensor, int, float]],
        optimize_inner_acqf_config: Dict,
        bounds: Optional[torch.Tensor],
        delta: Optional[Union[int, float]],
        epsilon_diff_acq_value: Optional[Union[int, float]],
        generate_initial_data: Optional[
        ...
        self.delta = delta
        self.update_bounds = self.bounds is None
        self.optimize_inner_acqf_config = optimize_inner_acqf_config
        self.max_samples_per_iteration = max_samples_per_iteration
        self.verbose = verbose
        self.wandb_run = wandb_run
...
class LAMinUCB(AbstractOptimizer):
    ...
    def __init__(
        ...
        # Add initialization parameter to training data.
        train_x_init = torch.cat([train_x_init, self.params])
        self.params_value=self.objective(self.params)

        train_y_init = torch.cat(
            [train_y_init, self.params_value.reshape(-1, 1)]
        )
        ...
        # Acquistion function and its optimization properties.
        self.acqf_config = acqf_config
        self.acquisition_function = Local_KG(self.model,**acqf_config)
        self.optimize_acqf_config = optimize_acqf_config
        self.optimize_inner_acqf_config = optimize_inner_acqf_config
        self.verbose = verbose
        f_params = self.objective(self.params)
        ...
        if self.verbose:
            print(f"Reward of parameters theta_(t-1): {f_params.item():.2f}.")
        self.model.append_train_data(self.params, f_params)
        self.params=self.params.squeeze(0)
    ...
    def step(self) -> None:
        ...
        # Optimize acquistion function and get new observation.
        new_x, _ = self.optimize_acqf(
            self.acquisition_function,
            self.bounds,
        )
        # print(new_x)
        batch_size=self.optimize_acqf_config['q']
        new_y=torch.empty(0)
        for i in range(batch_size):
            tempt_sample=self.objective((new_x[i,:]).unsqueeze(0))
            new_y=torch.cat(
            [new_y, tempt_sample])
            if tempt_sample>self.params_value:
                self.params=(new_x[i,:])
                self.params_value=tempt_sample
                self.flag=1
        # print(new_y)
        
        # if self.params_value<0 and new_y>self.params_value:
        #     self.params.data=new_x
        #     self.params_value=new_y
        #     self.update_bounds=True
        self.model.append_train_data(new_x, new_y)
        self.iteration+=1
        ...
        # self.model.set_train_data(inputs=train_x, targets=train_y, strict=False)
        initial_point=self.params.unsqueeze(0)
        if self.iteration% self.explore_interval==0:
            acq_local=Local_acquisition_1(model=self.model)
            best_x,_=optimize_acqf(
            acq_local,
            batch_initial_conditions=initial_point.detach(),
            **self.optimize_inner_acqf_config
            ...
            best_x_r=best_x.unsqueeze(0)
            # tempt=self.objective(best_x.clone())
            ...
            self.model.set_train_data(inputs=train_x, targets=train_y, strict=False)
            print("current steps = ",self.iteration)
            print("current best y = ",self.params_value)
            if self.optimize_hyperparamters:
                mll = gpytorch.mlls.ExactMarginalLogLikelihood(
                    self.model.likelihood, self.model
                )
...
class TURBO(AbstractOptimizer):
    ...
    def __init__(
        ...
        """Inits the vanilla BO optimizer."""
        super(TURBO, self).__init__(params_init, objective)
        ...
        self.failure_tolerance = math.ceil(
            max([4.0 / self.batch_size, float(self.D) / self.batch_size])
        )
    ...
    def step(self) -> None:
        ...
        # Optimize acquistion function and get new observation.
        new_x = self.generate_batch(self.model,self.model.train_inputs[0],self.model.train_targets,self.batch_size)
        new_y = self.objective(new_x)
        self.params = new_x.clone()
        self.update_state(self.model.train_targets)
        # Update training points.
        train_x = torch.cat([self.model.train_inputs[0], new_x])
        train_y = torch.cat([self.model.train_targets, new_y])
        self.model.set_train_data(inputs=train_x, targets=train_y, strict=False)

...
from src.optimizer_for_local_kg import optimize_acqf_local_kg
...
def optimize_acqf_custom_bo(
    ...
    candidates, acq_value = botorch.optim.optimize_acqf(
        ...
        options={"nonnegative": True, "batch_limit": 5,"maxiter":5},
...
def optimize_acqf_lkg(
    ...
    candidates, acq_value = optimize_acqf_local_kg(
        acq_function=acq_func,
        bounds=bounds,
        q=q,  # Analytic acquisition function.
        num_restarts=num_restarts,
        raw_samples=raw_samples,  # Used for initialization heuristic.
        options={"nonnegative": True, "batch_limit": 10,"maxiter":5},
        return_best_only=True,
        sequential=False,
    )
    # Observe new values.
